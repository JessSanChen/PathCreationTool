...
function was added so that expensive singular operations only need to be
    ...
    methods
        function obj = draggable(g, varargin)
            ...
            % *** on_click
            obj.on_click_callback = args.on_click_callback;
...
%                 '-not', 'Type','hgtransform', '-not', 'Type', 'axes');
%             
%             % if selected is part of transformed group. store in
%             % axes.UserData
%             transformed = [];
%             if isequal(obj.g.Parent.Type, "hgtransform")
%                 transformed = findobj(obj.g.Parent,'-not','Type','hgtransform');
%                 obj.axh.UserData = obj.g.Parent;
%             else
%                 obj.axh.UserData = obj.g;
%             end
% 
%             % set selected as red, all other objects in uiaxes blue
%             for i = 1:length(allchildren)
%                 % if part of hgtransform group
%                 if ismember(allchildren(i), transformed)
%                     allchildren(i).Color = 'r';
%                 % if just by itself (point)
%                 elseif isequal(allchildren(i), obj.g)
%                     allchildren(i).Color = 'r';
%                 else
...
%             end
    ...
    methods (Access = private, Static)
        ...
        function args = parse(g, varargin)
            ...
            % *** try making on_click_callback
            on_click_callback = @(varargin)[];
            ...
            for k = 1:nargin - 1
                ...
                % *** on_click testing
                elseif isa(current_arg, "function_handle") && on_move_done
                    on_click_callback = current_arg;
            ...
            % *** on_click
            args.on_click_callback = on_click_callback;
