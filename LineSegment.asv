...
%         end
        ...
        function lengthen(obj, point, newLength)
            ...
            % determine endpoint from point
            endp = obj.getEndp(point);

            oldLength = endp.Pos;

%             dir = obj.getDir(endp);
            angle = dir(endp.Pos, obj.otherP(endp).Pos);

            obj.otherP(endp).Pos = [endp.Pos(1) + (newLength) * cosd(angle), ...
                       endp.Pos(2) + (newLength) * sind(angle)];
